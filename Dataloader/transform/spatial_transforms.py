...
class VideoRandomCrop(object):
    ...
    def __init__(self, size):
        self.size = size
        self.from_y = int(160-(size/2))
        self.from_x = int(120-(size/2))
        self.to_y = self.from_y + size
        self.to_x = self.from_x + size

    def __call__(self, clip):
        clip = clip[:,:,self.from_x:self.to_x,self.from_y:self.to_y]
