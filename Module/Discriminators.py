...
from Module.SpectralConvLSTM import SpectralConvLSTM
...
class DiscriminatorLSTM(nn.Module):
    def __init__(self, in_channels=3):
        ...
        self.lstm_network_1 = SpectralConvLSTM(128, 256, (3,3), 1, batch_first=True)
        self.a1 = nn.LeakyReLU()
        self.pool1 = nn.AvgPool2d(2)
        self.lstm_network_2 = SpectralConvLSTM(256, 512, (3,3), 1, batch_first=True)
        self.a2 = nn.LeakyReLU()
        self.pool2 = nn.AvgPool2d(2)
        self.linear = SpectralNorm(nn.Linear(512, 1))
    def forward(self,x):
        B,T,C,H,W = x.shape
        x = x.view(B*T,C,H,W)
        out = self.pre_conv(x)
        ...
        #out = self.a1(self.lstm_network_1(out, torch.flip(out,[1])))
        out = self.a1(self.lstm_network_1(out)[0][0])
        B,T,C,H,W = out.shape
        out = self.pool1(out.view(B*T,C,H,W))
        _,C,H,W = out.shape
        out = out.view(B,T,C,H,W)
        #out = self.a2(self.lstm_network_2(out, torch.flip(out,[1])))
        out = self.a2(self.lstm_network_2(out)[0][0])
        B,T,C,H,W = out.shape
        out = self.pool2(out.view(B*T,C,H,W)).view(B*T,C,-1)
        # _,C,H,W = out.shape
        # out = out.view(B,T,C,H,W)
        # #out = self.a3(self.lstm_network_3(out, torch.flip(out,[1])))
        # out = self.a3(self.lstm_network_3(out)[0][0])
        # B,T,C,H,W = out.shape
        # out = self.pool3(out.view(B*T,C,H,W)).view(B*T,C,-1)
        out = out.sum(2)
        out = self.linear(out).squeeze(1)
        return out
        
class Discriminator_space(nn.Module):
    def __init__(self, in_channels=3):
        super().__init__()
        self.pre_conv = nn.Sequential(SpectralNorm(nn.Conv2d(in_channels,64,3,padding='same')), 
                                       nn.LeakyReLU(),
                                       SpectralNorm(nn.Conv2d(64,128,3,padding='same')), 
                                       nn.LeakyReLU(),
                                       SpectralNorm(nn.Conv2d(128,128,3,padding='same')),
                                       nn.LeakyReLU(),
                                       nn.AvgPool2d(2),
                                      SpectralNorm(nn.Conv2d(128,256,3,padding='same')),
                                       nn.LeakyReLU(),
                                       nn.AvgPool2d(2),
                                      SpectralNorm(nn.Conv2d(256,256,3,padding='same')),
                                       nn.LeakyReLU(),
                                       nn.AvgPool2d(2),
