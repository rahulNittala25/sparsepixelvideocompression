...
class SpectralConvGRUCell(nn.Module):
    """
    ...
    def __init__(self, input_size, hidden_size, kernel_size, activation=torch.sigmoid):
        ...
        self.reset_gate = SpectralNorm(nn.Conv2d(input_size + hidden_size, hidden_size, kernel_size, padding=padding))
        self.update_gate = SpectralNorm(nn.Conv2d(input_size + hidden_size, hidden_size, kernel_size, padding=padding))
        self.out_gate = SpectralNorm(nn.Conv2d(input_size + hidden_size, hidden_size, kernel_size, padding=padding))
        self.activation = activation

        init.orthogonal_(self.reset_gate.module.weight)
        init.orthogonal_(self.update_gate.module.weight)
        init.orthogonal_(self.out_gate.module.weight)
        init.constant_(self.reset_gate.module.bias, 0.)
        init.constant_(self.update_gate.module.bias, 0.)
        init.constant_(self.out_gate.module.bias, 0.)
...
class SpectralConvGRU(nn.Module):
    ...
    def __init__(self, input_size, hidden_sizes, kernel_sizes, n_layers):
        ...
        for i in range(self.n_layers):
            ...
            cell = SpectralConvGRUCell(input_dim, self.hidden_sizes[i], self.kernel_sizes[i])
...
if __name__ == "__main__":
    ...
    model = SpectralConvGRU(input_size=8, hidden_sizes=[32, 64, 16], kernel_sizes=[3, 5, 3], n_layers=3).cuda()
