...
#                                             (self.beta1, self.beta2))
    ...
    def epoch2step(self):
        ...
        self.log_step =  200 #self.log_epoch * step_per_epoch
    ...
    def train(self):
        ...
        for step in range(start, self.total_step):
            ...
            for i in range(self.d_iters):
                ...
                #video_mask = self.mask_G(real_videos)
                
                B,T,C,H,W = real_videos.shape
                video_mask = torch.rand((B,T,1,H,W))
                video_mask = (video_mask<self.density)*1.0
                ...
                # ============== Frame Generator ================= #
                b,t,c,h,w = real_videos.shape
                seed2 = torch.rand((b,t,c,h,w)).to(self.device)
                ...
                selected_density = ((torch.sum(new_mask)/ new_mask.shape[0]) - self.density)
                if(selected_density<self.density):
                    d_loss = 10*(selected_density/self.density)**2
                else:
                    d_loss = (selected_density)**2
                ...
                g_loss = ds_loss_fake + dt_loss_fake + 5*mse_loss + d_loss
...
#         self.D_t = TemporalDiscriminator(chn=self.dt_chn, n_class=self.n_class).cuda()
        ...
        # self.mask_G = Mask_Generator_Net().to(self.device)
